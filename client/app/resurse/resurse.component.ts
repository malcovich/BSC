...
export class ResurseComponent implements OnInit {
  ...
  getSameItems(obj) {
    this.arrayT = [];
    this.resurseService.getSameItems(obj).subscribe(res => {
      this.sameRes = res.same.map(item => {
        if (item.resurse == "hintwise") {
          this.hintwise = item;
        }
  ...
  saveResult(item) {
    this.resurseService
      .saveResult({
        ...
        systemPredicion: this.match.prediction
      })
      .subscribe(date => {
  ...
  addToFavorite() {
    var obj = {
      id: this.match._id,
      isFavorite: !this.match.isFavorite,
      systemPredicion: this.match.prediction
    };
    this.resurseService.addToFavorite(obj).subscribe(resurse => {
      this.match.isFavorite = resurse.isFavorite;
    });
  }

  getPrediction() {
    if (
      this.arrayT.indexOf("12") > -1 ||
      ((this.arrayT.indexOf("1") > -1 || this.arrayT.indexOf("1X") > -1) &&
        (this.arrayT.indexOf("2") > -1 || this.arrayT.indexOf("X2") > -1))
    ) {
      this.match.prediction = "NO PREDICTION";
    } else if (this.arrayT.indexOf("1X") > -1) {
      this.match.prediction = "1X";
    } else if (
      this.arrayT.indexOf("2") > -1 &&
      (this.arrayT.indexOf("X2") > -1 || this.arrayT.indexOf("X") > -1)
    ) {
      this.match.prediction = "X2";
    } else if (
      this.arrayT.indexOf("1") > -1 &&
      (this.arrayT.indexOf("1X") > -1 || this.arrayT.indexOf("X") > -1)
    ) {
      this.match.prediction = "1X";
  ...
  getClubsInfo(obj) {
    this.resurseService.getClubsInfo(obj).subscribe(data => {
    ...
    if (this.team1 && this.team2) {
      ...
      if (
        (this.team1.name !== team1 &&
          this.team1.simpleNames.indexOf(team1) == -1) ||
        (this.team2.name !== team2 &&
          this.team2.simpleNames.indexOf(team2) == -1)
      ) {
        return true;
      }
    } else {
  ...
  saveSimpleNames(team1, team2) {
    this.resurseService
      .addSimpleNames({
        simpleName1: team1,
        simpleName2: team2,
        idTeam1: this.team1._id,
        idTeam2: this.team2._id
      })
      .subscribe(data => {
        this.team1 = data.team1;
        this.team2 = data.team2;
  ...
  setUnifyPrediciton(item) {
    ...
    if (
      (item.prediction == undefined || item.prediction == "") &&
      item.correctScore
    ) {
